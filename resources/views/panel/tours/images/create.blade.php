...
@section('content')
@include('panel.include.block-header.min', ['data' => ['sub' => __('Add image'), 'title' => $tour->title]])

<form action="{{ route('panel.tours.images.store', ['tour' => $tour->id, 'type' => $type]) }}" class="form-contact needs-validation" method="POST" enctype="multipart/form-data" novalidate>
  @csrf
