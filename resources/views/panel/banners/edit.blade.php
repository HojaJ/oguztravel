...
@section('title')
{{ __('Edit thing', ['thing' => __('Banner')]) }}
@endsection

@section('content')
@include('panel.include.block-header.min', ['data' => ['sub' => __('Edit thing', ['thing' => __('Banner')]), 'title' => $banner->title]])

<form action="{{ route('panel.banners.update', $banner->id) }}" class="form-contact needs-validation mt-4" method="POST" enctype="multipart/form-data" novalidate>
  @csrf
  @method('PATCH')
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="form-control-wrap">
              <input type="text" id="title-{{ $localeCode }}" name="title[{{ $localeCode }}]" value="{{ old('title.' . $localeCode, $banner->getTranslation('title', $localeCode)) }}" class="form-control form-control-lg @error('title.' . $localeCode) is-invalid @enderror" placeholder="{{ $properties['native'] }}" required>
      ...
      <div class="row g-4">
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="form-control-wrap">
              <input type="text" id="subtitle-{{ $localeCode }}" name="subtitle[{{ $localeCode }}]" value="{{ old('subtitle.' . $localeCode, $banner->getTranslation('subtitle', $localeCode)) }}" class="form-control form-control-lg @error('subtitle.' . $localeCode) is-invalid @enderror" placeholder="{{ $properties['native'] }}">
  ...
  <div class="form-group">
    ...
    <div class="form-control-wrap">
      <input type="text" id="link" name="link" class="form-control form-control-lg @error('link') is-invalid @enderror" placeholder="{{ __('URL') }}" value="{{ old('link', $banner->link) }}" required>
      @if ($errors->has('link'))
