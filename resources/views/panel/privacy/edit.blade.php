...
@section('title') {{ __('Edit thing', ['thing' => __($privacy->page)]) }} @endsection

@section('content')
@include('panel.include.block-header.min', ['data' => ['sub' => __('Edit thing', ['thing' => __($privacy->page)]), 'title' => __($privacy->page)]])

<form action="{{ route('panel.privacy.update', $privacy->id) }}" class="form-contact needs-validation" method="POST" enctype="multipart/form-data" novalidate>
  @csrf
  @method('patch')
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="form-control-wrap">
              <input type="text" id="title-{{ $localeCode }}" name="title[{{ $localeCode }}]" value="{{ old('title.' . $localeCode, $privacy->getTranslation('title', $localeCode)) }}" class="form-control form-control-lg @error('title.' . $localeCode) is-invalid @enderror" placeholder="{{ $properties['native'] }}" required>
      ...
      <div class="row g-4">
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="form-control-wrap">
              <input type="text" id="subtitle-{{ $localeCode }}" name="subtitle[{{ $localeCode }}]" value="{{ old('subtitle.' . $localeCode, $privacy->getTranslation('subtitle', $localeCode)) }}" class="form-control form-control-lg @error('subtitle.' . $localeCode) is-invalid @enderror" placeholder="{{ $properties['native'] }}" required>
  ...
  <div class="card card-bordered mb-2">
    <div class="card-inner">
      ...
      <div class="form-group">
        ...
        <div class="form-control-wrap">
          <textarea id="desc-{{ $localeCode }}" name="desc[{{ $localeCode }}]" class="form-control editor @error('desc.' . $localeCode) is-invalid @enderror" placeholder="{{ $properties['native'] }}" required>{{ old('desc.' . $localeCode, $privacy->getTranslation('desc', $localeCode)) }}</textarea>
